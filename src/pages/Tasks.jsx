...
const Tasks = () => {
  ...
  const upNextTask=tasks.tasks.filter((task)=>task?.status==="pending")
  const inProgress=tasks.tasks.filter((task)=>task?.status==="inProgress")
      ...
      <div className="col-span-9 px-10 pt-10">
        <div className="flex justify-between items-center">
          ...
          <div className="flex gap-5">
            ...
            <AddTask isOpen={isOpen} setIsOpen={setIsOpen} title={"Brohh I can Do it"} >Hi</AddTask>
        ...
        <div className="grid grid-cols-3 gap-5 mt-10">
          <div className="relative h-[800px] overflow-auto">
            ...
            <div className="space-y-3">
              {
                tasks&&upNextTask.map(task=><TaskCard key={task.id} task={task} Status={"inProgress"} />)
              }
          ...
          <div className="relative h-[800px] overflow-auto">
            ...
            <div className="space-y-3">
            {
                tasks&&inProgress.map(task=><TaskCard key={task.id} task={task} Status={"done"} />)
              }
            </div>
          </div>
          <div className="relative h-[800px] overflow-auto">
            ...
            <div className="space-y-3">
            {
                tasks&&doneTasks.map(task=><TaskCard key={task.id} task={task} Status={"archive"}  />)
              }
            </div>
          </div>
        </div>
